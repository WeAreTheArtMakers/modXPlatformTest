...
contract MODXStaking is ReentrancyGuard, Ownable, Pausable {
    ...
    function unstake(uint256 poolId) external nonReentrant {
        ...
        // unstake fonksiyonu içinde, stake sıfırlandıysa array'den poolId silinmesi:
        if (userStake.amount == 0) {
            uint256[] storage arr = userPoolIds[msg.sender];
    ...
    function getUserStakeInfo(address user, uint256 poolId) external view returns (
        ...
        UserStake memory us = userStakes[user][poolId];
        return (
            us.amount,
            us.stakeTime,
            us.lockEndTime,
            getPendingRewards(user, poolId),
            (block.timestamp >= us.lockEndTime && us.amount > 0)
        );
    }
    
    // eklenen fonksiyon
    function userActivePoolIds(address user) external view returns (uint256[] memory) {
        uint256[] memory allPools = userPoolIds[user];
        uint256 activeCount = 0;
        for (uint256 i = 0; i < allPools.length; i++) {
            if (userStakes[user][allPools[i]].amount > 0) {
                activeCount++;
            }
        }
